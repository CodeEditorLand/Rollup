		...
		renderedChunks.map(
			async ({
				...
				const transformedChunk: RenderedChunkWithPlaceholders = {
					...
					fileName,
					sourcemapFileName:
						preliminarySourcemapFileName?.fileName ?? null,
				...
				if (hashPlaceholder) {
					...
					renderedChunksByPlaceholder.set(
						hashPlaceholder,
						transformedChunk,
					);
				...
				if (map && sourcemapHashPlaceholder) {
					initialHashesByPlaceholder.set(
						preliminarySourcemapFileName.hashPlaceholder,
						getXxhash(map.toString()).slice(
							0,
							preliminarySourcemapFileName.hashPlaceholder.length,
						),
					);
				}
			},
...
function generateFinalHashes(
	...
	for (const [placeholder, { fileName }] of renderedChunksByPlaceholder) {
		...
		do {
			...
			finalFileName = replaceSinglePlaceholder(
				fileName,
				placeholder,
				finalHash,
			);
...
function addChunksToBundle(
	...
	for (const {
		...
		const finalFileName = replacePlaceholders(
			fileName,
			hashesByPlaceholder,
		);
