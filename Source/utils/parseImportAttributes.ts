...
export function getAttributesFromImportExpression(
	...
	const assertProperty = options.properties.find(
	...
	if (!(assertProperty instanceof ObjectExpression)) {
		context.module.log(
			LOGLEVEL_WARN,
			logImportOptionsAreInvalid(context.module.id),
			start,
		);
		return EMPTY_OBJECT;
	}
	const assertFields = (assertProperty as ObjectExpression).properties
		.map((property) => {
			...
			if (
				typeof key === "string" &&
				typeof ((property as Property).value as Literal).value ===
					"string"
			) {
				return [
					key,
					((property as Property).value as Literal).value,
				] as [string, string];
			}
			context.module.log(
				LOGLEVEL_WARN,
...
const getPropertyKey = (
	...
	return (
		...
		((key as Identifier).name || (key as Literal).value)
	);
};
...
export function getAttributesFromImportExportDeclaration(
	...
	return attributes?.length
		? Object.fromEntries(
				attributes.map((assertion) => [
					getPropertyKey(assertion),
