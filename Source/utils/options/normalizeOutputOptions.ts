...
export async function normalizeOutputOptions(
	...
	const externalImportAttributes = getExternalImportAttributes(
		config,
		inputOptions,
	);
	...
	const outputOptions: NormalizedOutputOptions & OutputOptions = {
		amd: getAmd(config),
		assetFileNames:
			config.assetFileNames ?? "assets/[name]-[hash][extname]",
		...
		minifyInternalExports: getMinifyInternalExports(
			config,
			format,
			compact,
		),
	...
	return mergedOption.autoId
		? {
				...
				forceJsExtensionForImports:
					mergedOption.forceJsExtensionForImports,
			}
		: {
				autoId: false,
				define: mergedOption.define,
...
const getExternalImportAttributes = (
	...
	return (
		config.externalImportAttributes ??
		config.externalImportAssertions ??
		true
	);
};
...
const getIndent = (
	config: OutputOptions,
	compact: boolean,
): NormalizedOutputOptions["indent"] => {
	if (compact) {
		return "";
	}
...
const getInterop = (
	...
	if (typeof configInterop === "function") {
		...
		return (id) =>
			id === null
				? defaultInterop ||
					validateInterop((defaultInterop = configInterop(id)))
				: id in interopPerId
					? interopPerId[id]
	...
	if (!ALLOWED_INTEROP_TYPES.has(interop)) {
		return error(
			logInvalidOption(
				...
				`use one of ${Array.from(ALLOWED_INTEROP_TYPES, (value) =>
