...
const UNKNOWN_DEOPTIMIZED_ENTITY = new Set<ExpressionEntity>([
	UNKNOWN_EXPRESSION,
]);
...
export default class ParameterVariable extends LocalVariable {
	...
	addEntityToBeDeoptimized(entity: ExpressionEntity): void {
		if (entity === UNKNOWN_EXPRESSION) {
			...
			for (const { interaction, path } of this
				.deoptimizationInteractions) {
			...
			(path.length === 1 &&
				(this.deoptimizedFields.has(UnknownKey) ||
					(interaction.type === INTERACTION_CALLED &&
						this.deoptimizedFields.has(path[0]))))
		) {
			deoptimizeInteraction(interaction);
			return;
		}
