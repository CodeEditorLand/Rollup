...
import { NodeBase, type ExpressionNode, type IncludeChildren } from "./Node";
...
export default class ClassNode extends NodeBase implements DeoptimizableEntity {
	...
	hasEffects(context: HasEffectsContext): boolean {
		...
		const initEffect =
			this.superClass?.hasEffects(context) ||
			this.body.hasEffects(context);
		...
		return interaction.type === INTERACTION_CALLED && path.length === 0
			? !interaction.withNew ||
					(this.classConstructor === null
						? this.superClass?.hasEffectsOnInteractionAtPath(
								path,
								interaction,
								context,
							)
						: this.classConstructor.hasEffectsOnInteractionAtPath(
								path,
								interaction,
								context,
							)) ||
					false
			: this.getObjectEntity().hasEffectsOnInteractionAtPath(
					path,
					interaction,
					context,
				);
	...
	protected applyDeoptimizations(): void {
		...
		for (const definition of this.body.body) {
			if (
				!(
					definition.static ||
					(definition instanceof MethodDefinition &&
