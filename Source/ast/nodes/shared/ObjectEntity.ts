...
export class ObjectEntity extends ExpressionEntity {
	...
	private readonly additionalExpressionsToBeDeoptimized =
		new Set<ExpressionEntity>();
	private readonly allProperties: ExpressionEntity[] = [];
	private readonly deoptimizedPaths: Record<string, boolean> =
		Object.create(null);
	...
	private readonly propertiesAndGettersByKey: PropertyMap =
		Object.create(null);
	private readonly propertiesAndSettersByKey: PropertyMap =
		Object.create(null);
	...
	deoptimizeAllProperties(noAccessors?: boolean): void {
		const isDeoptimized =
			this.hasLostTrack || this.hasUnknownDeoptimizedProperty;
		if (noAccessors) {
			...
			if (propertiesForExactMatchByKey[key]) {
				...
				if (!this.immutable) {
					for (const argument of args) {
						if (argument) {
							this.additionalExpressionsToBeDeoptimized.add(
								argument,
							);
						}
					}
	...
	deoptimizeIntegerProperties(): void {
		...
		for (const [key, propertiesAndGetters] of Object.entries(
			this.propertiesAndGettersByKey,
		)) {
			if (INTEGER_REG_EXP.test(key)) {
	...
	deoptimizePath(path: ObjectPath): void {
		...
		this.prototypeExpression?.deoptimizePath(
			path.length === 1 ? [...path, UnknownKey] : path,
		);
	}
	...
	hasEffectsOnInteractionAtPath(
		...
		const [
			propertiesAndAccessorsByKey,
			accessorsByKey,
			unmatchableAccessors,
		] =
			...
			if (propertiesAndAccessorsByKey[key]) {
				...
				if (accessors) {
					for (const accessor of accessors) {
						if (
							accessor.hasEffectsOnInteractionAtPath(
								subPath,
								interaction,
								context,
							)
	...
	private deoptimizeCachedEntities() {
		for (const expressionsToBeDeoptimized of Object.values(
