		...
		const processedOptions = await handler.call(
			{
				...
				error: (error_): never =>
					error(
						logPluginError(normalizeLog(error_), name, {
							hook: "onLog",
						}),
					),
				info: getLogHandler(
...
async function getOutputOptionsAndPluginDriver(
	...
	const outputPluginDriver =
		inputPluginDriver.createOutputPluginDriver(rawPlugins);
...
function getOutputOptions(
	...
	return normalizeOutputOptions(
		outputPluginDriver.hookReduceArg0Sync(
			...
			(pluginContext) => {
				const emitError = () =>
					pluginContext.error(logCannotEmitFromOptionsHook());
				return {
					...pluginContext,
					emitFile: emitError,
...
function createOutput(outputBundle: OutputBundle): RollupOutput {
	return {
		output: (
			Object.values(outputBundle).filter(
				(outputFile) => Object.keys(outputFile).length > 0,
			) as (OutputChunk | OutputAsset)[]
		).sort(
			(outputFileA, outputFileB) =>
				getSortingFileType(outputFileA) -
				getSortingFileType(outputFileB),
...
export function defineConfig<
	T extends RollupOptions | RollupOptions[] | RollupOptionsFunction,
>(options: T): T {
	return options;
}
