...
const [mainPackage, mainLockFile, browserPackage, repo, changelog] =
	await Promise.all([
...
try {
	...
	console.error(
		`Error during release, rolling back changes: ${error.message}`,
	);
...
function getNewLogEntry(version, prs) {
	...
	if (patch === 0) {
		...
		if (minor === 0) {
			sections =
				getDummyLogSection("Breaking Changes", firstPr) + sections;
		}
	}
	return `## ${version}
...
async function waitForChangelogUpdate(version) {
	...
	while (true) {
		...
		console.log(
			cyan("You generated the following changelog entry:\n") +
				changelogEntry,
		);
...
function updatePackages(mainPackage, mainLockFile, browserPackage, newVersion) {
	return Promise.all([
		writeFile(
			MAIN_PACKAGE,
			updatePackageVersionAndGetString(mainPackage, newVersion),
		),
		writeFile(
			MAIN_LOCKFILE,
			updateLockFileVersionAndGetString(mainLockFile, newVersion),
		),
		writeFile(
			BROWSER_PACKAGE,
			updatePackageVersionAndGetString(browserPackage, newVersion),
		),
	]);
}
