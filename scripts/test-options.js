...
const [optionsText, helpText, commandReferenceText] = await Promise.all([
	readFile(
		new URL("../docs/configuration-options/index.md", import.meta.url),
		"utf8",
	),
	readFile(new URL("../cli/help.md", import.meta.url), "utf8"),
	readFile(
		new URL("../docs/command-line-interface/index.md", import.meta.url),
		"utf8",
	),
]);
...
for (const section of optionSections.slice(1)) {
	if (
		!(
			section.startsWith("Experimental") ||
			section.startsWith("Deprecated")
		)
	) {
...
for (const { long, short } of allCliOptions) {
	if (!cliOptionsInHelp.has(long)) {
		failed = true;
		console.error(
			`Could find neither --${long} nor --no-${long} in help.md.`,
		);
	}
...
for (const line of splitHelpText) {
	if (line.length > 80) {
		console.error(
			`The following line in help.md exceeds the limit of 80 characters:\n${line}`,
		);
		exit(1);
	}
}
