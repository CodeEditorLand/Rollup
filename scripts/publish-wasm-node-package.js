...
const PACKAGE_DIR = fileURLToPath(
	new URL("../wasm-node-package", import.meta.url),
);
...
export default async function publishWasmNodePackage() {
	...
	await Promise.all([
		...COPIED_FILES_OR_DIRS.map((file) =>
			cp(file, getOutputPath(file), { recursive: true }),
		),
		writeFile(
	...
	const nativeJsContent = await readFile(
		new URL("../native.wasm.js", import.meta.url),
		"utf8",
	);

	await Promise.all([
		writeFile(
			getOutputPath("dist", "native.js"),
			nativeJsContent.trimStart(),
		),
		cp(
			"artifacts/bindings-wasm-node/wasm-node",
			getOutputPath("dist", "wasm-node"),
			{
				recursive: true,
			},
		),
