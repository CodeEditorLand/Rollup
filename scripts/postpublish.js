...
if (!matched) {
	throw new Error(
		`The last commit message "${newVersion}" does not contain a version.`,
	);
}
...
async function postReleaseComments(includedPRs, issues, version) {
	...
	const addComment = (issueNumber, comment) =>
		...
		new Promise((resolve) => setTimeout(resolve, 500)).then(() =>
			issues
				.createIssueComment(
					issueNumber,
					`${comment} as part of rollup@${version}. ${installNote}`,
				)
				.catch((error) => {
					console.error(error);
					caughtError ||= error;
	...
	for (const { pr, closed } of includedPRs) {
		...
		for (const closedIssue of closed) {
			await addComment(
				closedIssue,
				`This issue has been resolved via #${pr}`,
			);
			console.log(
				cyan(`Added fix comment to #${closedIssue} via #${pr}.`),
			);
		}
	}
	if (caughtError) {
